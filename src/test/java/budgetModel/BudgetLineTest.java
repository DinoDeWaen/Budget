...
public class BudgetLineTest {
	...
	public class YearlyFrequencyContext {
		...
		private void validateYearlyBudgetLine(BudgetLine budgetLine) {
			assertEquals(amount,
					budgetLine.calculateBudgetAmountInInterval(new Interval(
							beginDate, endDate)), ACCURACY);
			assertEquals(amount / 12,
					budgetLine.calculateMonthlyBudgetAmount(), ACCURACY);
			assertEquals(amount, budgetLine.calculateYearlyBudgetAmount(),
					ACCURACY);
		}

		private void validateYearlyCashFlow(BudgetLine budgetLine) {
			assertEquals(
					addSignToAmount(amount),
					budgetLine
							.calculateSignedBudgetAmountInInterval(new Interval(
									beginDate, endDate)), ACCURACY);
			assertEquals(addSignToAmount(amount) / 12,
					budgetLine.calculateMonthlySignedBudgetAmount(), ACCURACY);
			assertEquals(addSignToAmount(amount),
					budgetLine.calculateYearlySignedBudgetAmount(), ACCURACY);
		}
	}
	...
	public class HalfYearlyFrequencyContext {
		...
		private void validateHalfYearlyBudgetLine(BudgetLine budgetLine) {
			assertEquals(amount * 2,
					budgetLine.calculateBudgetAmountInInterval(new Interval(
							beginDate, endDate)), ACCURACY);
			assertEquals(amount / 6, budgetLine.calculateMonthlyBudgetAmount(),
					ACCURACY);
			assertEquals(amount * 2, budgetLine.calculateYearlyBudgetAmount(),
					ACCURACY);
		}

		private void validateHalfYearlyCashFlow(BudgetLine budgetLine) {
			assertEquals(
					addSignToAmount(amount) * 2,
					budgetLine
							.calculateSignedBudgetAmountInInterval(new Interval(
									beginDate, endDate)), ACCURACY);
			assertEquals(addSignToAmount(amount) / 6,
					budgetLine.calculateMonthlySignedBudgetAmount(), ACCURACY);
			assertEquals(addSignToAmount(amount) * 2,
					budgetLine.calculateYearlySignedBudgetAmount(), ACCURACY);
		}
	...
	public class TrimesterFrequencyContext {
		...
		private void validateTrimesterBudgetLine(BudgetLine budgetLine) {
			assertEquals(amount * 3,
					budgetLine.calculateBudgetAmountInInterval(new Interval(
							beginDate, endDate)), ACCURACY);
			assertEquals(amount / 4, budgetLine.calculateMonthlyBudgetAmount(),
					ACCURACY);
			assertEquals(amount * 3, budgetLine.calculateYearlyBudgetAmount(),
					ACCURACY);
		}

		private void validateTrimesterCashFlow(BudgetLine budgetLine) {
			assertEquals(
					addSignToAmount(amount) * 3,
					budgetLine
							.calculateSignedBudgetAmountInInterval(new Interval(
									beginDate, endDate)), ACCURACY);
			assertEquals(addSignToAmount(amount) / 4,
					budgetLine.calculateMonthlySignedBudgetAmount(), ACCURACY);
			assertEquals(addSignToAmount(amount) * 3,
					budgetLine.calculateYearlySignedBudgetAmount(), ACCURACY);
		}
	...
	public class QuarterlyFrequencyContext {
		...
		private void validateQuarterlyBudgetLine(BudgetLine budgetLine) {
			assertEquals(amount * 4,
					budgetLine.calculateBudgetAmountInInterval(new Interval(
							beginDate, endDate)), ACCURACY);
			assertEquals(amount / 3, budgetLine.calculateMonthlyBudgetAmount(),
					ACCURACY);
			assertEquals(amount * 4, budgetLine.calculateYearlyBudgetAmount(),
					ACCURACY);
		}

		private void validateQuarterlyCashFlow(BudgetLine budgetLine) {
			assertEquals(
					addSignToAmount(amount) * 4,
					budgetLine
							.calculateSignedBudgetAmountInInterval(new Interval(
									beginDate, endDate)), ACCURACY);
			assertEquals(addSignToAmount(amount) / 3,
	...
	public class MonthlyBudgetContext {
		...
		private void validateMonthlyBudgetLine(BudgetLine budgetLine) {
			assertEquals(amount * 12,
					budgetLine.calculateBudgetAmountInInterval(new Interval(
							beginDate, endDate)), ACCURACY);
			assertEquals(amount, budgetLine.calculateMonthlyBudgetAmount(),
					ACCURACY);
			assertEquals(amount * 12, budgetLine.calculateYearlyBudgetAmount(),
					ACCURACY);
		}

		private void validateMonthlyCashFlow(BudgetLine budgetLine) {
			assertEquals(
					addSignToAmount(amount) * 12,
					budgetLine
							.calculateSignedBudgetAmountInInterval(new Interval(
									beginDate, endDate)), ACCURACY);
			assertEquals(addSignToAmount(amount),
	...
	private BudgetLine buildIncomeBudgetLine() {
		return BudgetLine
				...
				.withNumberOfMonthsBetweenDueDates(
						numberOfMonthsBetweenDueDates).build();
	}

	private void validateDates(BudgetLine budgetLine) {
		assertEquals(numberOfMonthsBetweenDueDates, budgetLine
				.getPeriodBetweenDueDates().getMonths());
