...
public class BalanceTest {
	...
	private static final Category incomeCategory = Category.newBuilder()
			.withCategoryName("income").build();
	private static final Category expenseCategory = Category.newBuilder()
			.withCategoryName("expense").build();

	private final double[] incomeAmounts = { 2000.00, 99.9, 200.34, 43.98 };
	private final double[] expenseAmounts = { 1000.00, 69.9, 20.34, 96.02 };

	private static Budget incomeBudget = BudgetCenter.newBuilder()
			.withName("income").withCategory(incomeCategory)
			.withBudgetLine(buildIncomeBudgetLine()).build();
	private static Budget expenseBudget = BudgetCenter.newBuilder()
			.withName("income").withCategory(expenseCategory)
			.withBudgetLine(buildExpenseBudgetLine()).build();
	...
	public void createBalanceTest() {
		...
		assertEquals(balance.calculateTotalIncome(),
				Arrays.stream(incomeAmounts).sum(), ACCURACY);
		assertEquals(balance.calculateTotalExpense(),
				Arrays.stream(expenseAmounts).sum() * -1.0, ACCURACY);
	}
}
