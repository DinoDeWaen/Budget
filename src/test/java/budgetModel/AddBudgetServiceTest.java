...
public class AddBudgetServiceTest {
	...
	private static final Category category = Category.newBuilder()
	...
	private BudgetDTO buildBudgetDTO() {
		return BudgetDTO
				...
				.withNumberOfMonthsBetweenDueDates(
						numberOfMonthsBetweenDueDates).withCategory(categoryId)
				.build();
	}
	...
	public class IncomeExpenseContext {
		...
		private void validateBudgetContent(BudgetCenter budget) {
			...
			assertEquals(amount * 12,
					budget.getBudgetAmount(new Interval(beginDate, endDate)),
					ACCURACY);
	...
	public class CategoryContext {
		public class EmptyCategoryContext {
			...
			private BudgetDTO buildBudgetWithoutCategoryDTO() {
				return BudgetDTO.newBuilder().withBeginDate(beginDate)
						.withEndDate(endDate).withDueDate(dueDate)
						.withAmount(amount).withName(name).build();
			}

			private void validateEmptyCategoryContent(BudgetCenter budget) {
				assertNotNull("category is null", budget.getCategory());

				assertEquals(Category.emptyCategory.getName(), budget
						.getCategory().getName());
			}
		}
	...
	public class AddMultipleCashFlowContext {
		...
		public void testSumOffCashFlows() {
			...
			assertEquals(
					Arrays.stream(incomeAmounts).sum()
							- Arrays.stream(expenseAmounts).sum(),
					budget.calculateCashFlowBalance(), ACCURACY);
		}
